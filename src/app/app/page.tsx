...
export interface EatOutSuggestion {
  id: string;
  name: string;
  commentary: string;
  mapsQuery: string;
...
export interface CookHomeSuggestion {
  id: string;
  recipeName: string;
  commentary: string;
...
export type Suggestion = EatOutSuggestion | CookHomeSuggestion;
...
export default function HomePage() {
  ...
  const fetchSuggestions = async () => {
    ...
    try {
      ...
      if (mode === "eatOut") {
        type EatOutRaw = Omit<EatOutSuggestion, "id">;
        const { data }: { data: EatOutRaw[] } = await response.json();

        const enriched: EatOutSuggestion[] = data.map((s, index) => ({
          ...s,
        ...
        type CookHomeRaw = Omit<CookHomeSuggestion, "id">;
        const { data }: { data: CookHomeRaw[] } = await response.json();

        const enriched: CookHomeSuggestion[] = data.map((s, index) => ({
          ...s,
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-8 bg-gradient-to-br from-white via-orange-50 to-orange-100">
        ...
        <AnimatePresence>
          {showSpinnerWheel && (
            <SpinnerWheel
              ...
              onSpinFinish={(s: Suggestion) => setSpunSuggestion(s)}
      ...
      <motion.footer className="mt-6 text-sm text-gray-500 text-center">
        &copy; {new Date().getFullYear()} LunchSpin.ai — Made with ❤️ by{" "}
        <a
